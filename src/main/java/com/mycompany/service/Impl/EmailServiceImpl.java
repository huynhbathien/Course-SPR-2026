...
public class EmailServiceImpl implements EmailService {
  ...
  public void sendOtpEmail(String to, String otpCode, OtpType otpType) {
    try {
      ...
      if (OtpType.EMAIL_VERIFICATION.equals(otpType)) {
        ...
        helper.setText(buildVerificationEmailHtml(otpCode, otpExpiryMinutes), true);
      } else {
        helper.setSubject("Reset Your Password - CourseApp");
        helper.setText(buildPasswordResetEmailHtml(otpCode, otpExpiryMinutes), true);
      }
  ...
  private String buildVerificationEmailHtml(String otpCode, int expiryMinutes) {
    return """
        <!DOCTYPE html>
        <html>
  ...
  private String buildPasswordResetEmailHtml(String otpCode, int expiryMinutes) {
    return """
        <!DOCTYPE html>
        <html>
        <head><meta charset="UTF-8"></head>
