...
import com.mycompany.util.CookieUtils;
...
public class AuthController {
    ...
    public APIResponse<Object> login(@Valid @RequestBody LoginRequestDTO dto,
        ...
        String clientIp = CookieUtils.resolveClientIp(request);
    ...
    @PostMapping("/refresh")
    public APIResponse<Object> refreshToken(HttpServletResponse response) {
        String username = resolveAuthenticatedUsername();
        Map<String, String> data = authService.refreshToken(username);
        setRefreshTokenCookie(response, data.get(REFRESH_TOKEN_COOKIE));
        return APIResponse.success(EnumSuccess.LOGIN_SUCCESS.getCode(),
                EnumSuccess.LOGIN_SUCCESS.getMessage(), data);
    }
    ...
    public APIResponse<Object> logout(HttpServletResponse response) {
        ...
        CookieUtils.addHttpOnlyCookie(response, REFRESH_TOKEN_COOKIE, "", 0, REFRESH_TOKEN_PATH);
    ...
    private void setRefreshTokenCookie(HttpServletResponse response, String token) {
        CookieUtils.addHttpOnlyCookie(response, REFRESH_TOKEN_COOKIE, token,
